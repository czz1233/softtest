...
define(function(require, exports, moudle) {
	...
	moudle.exports = {
			...
			tableInit:function(){
				...
				var url = basePath + "system/role/getTableData";
				param.columns = [
							{"data":"name"},
							{"data":"describle"},
							{"data":"createTime"},
				...
				param.columnDefs = [{"render":function ( data, type, row) {
				                    	...
				                    	var rightButton = $("<label>&nbsp;&nbsp;<button type='button' class='but-row-right btn btn-minier btn-success btn-round'><i class='fa fa-users bigger-110'></i>  授权</button><label>");
				                    	rightButton.children("button").attr({"vid":row.id});
				                    	...
				                    	var html = editButton.html() + rightButton.html() + deleteButton.html();
				                    	return html;
				                    },
				                    "targets":4}
			...
			addRecordShow:function(){
				var url = basePath + "system/role/addRecordShow";
				loadUtil.load(url);
			},
			editRecordShow:function(id){
				var url = basePath + "system/role/" + id +"/editRecordShow";
				loadUtil.load(url);
			},
				...
				winUtil.confirm("操作确认","您确定删除该条记录吗?",function(){
					$.ajax({
						...
						success:function(msg){
							if(msg.ok){
								util.showMsg("success","操作成功","您已经成功删除角色信息");
			...
			getRight:function(id){
				var url = basePath + "system/role/" + id +"/roleRightShow";
				loadUtil.load(url);
			},
			listener:function(){
				var items = [
				...
				$("#" + moudle.exports.tableId).on('click', '.but-row-right', function (event) {
					moudle.exports.getRight($(event.currentTarget).attr("vid"));
				});
			},
			init:function(){
