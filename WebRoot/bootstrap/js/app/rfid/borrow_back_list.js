...
define(function(require, exports, moudle) {
	...
	moudle.exports = {
			...
			tableInit:function(){
				...
				param.columnDefs = [{"render":function ( data, type, row) {
				                    ...
				                    {"render":function ( data, type, row) {
				                    	var auditButton = $("<label>&nbsp;&nbsp;<button type='button' class='but-row-audit btn btn-minier btn-success btn-round'><i class='fa fa-check bigger-110'></i>  归还</button><label>");
			...
			auditRecordShow:function(id){
				winUtil.confirm("操作确认","您确定归还该档案吗?",function(){
					$.ajax({
						url:basePath + "rfid/borrow/backBorrow",
						data:{"id":id},
						type:"POST",
						dataType:"JSON",
						success:function(msg){
							if(msg.ok){
								util.showMsg("success","操作成功","外借档案成功");
