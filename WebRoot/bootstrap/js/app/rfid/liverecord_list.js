...
define(function(require, exports, moudle) {
	...
	moudle.exports = {
			tableId:"liverecord_table",
			tableInit:function(){
				var param = {};
				param.tableId = moudle.exports.tableId;
				param.serverSide = false;
				var url = basePath + "rfid/liverecord/getTableData";
				param.columns = [
							{"data":"archiveTitle"},
							{"data":"archiveElTag"},
							{"data":"liveUser"},
			...
			addRecordShow:function(){
				var url = basePath + "rfid/liverecord/addRecordShow";
				$("#main-content").load(url);
			},
			editRecordShow:function(id){
				var url = basePath + "rfid/liverecord/" + id +"/editRecordShow";
				$("#main-content").load(url);
			},
			deleteRecord:function(id){
				winUtil.confirm("操作确认","您确定删除该条记录吗?",function(){
					$.ajax({
						url:basePath + "rfid/liverecord/deleteLiveRecord",
						data:{"id":id},
						type:"POST",
						dataType:"JSON",
						success:function(msg){
							if(msg.ok){
								util.showMsg("success","操作成功","您已经成功删除调档记录信息");
