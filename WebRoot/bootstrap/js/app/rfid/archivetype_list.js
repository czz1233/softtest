...
define(function(require, exports, moudle) {
	...
	moudle.exports = {
			tableId:"archivetype_table",
			tableInit:function(){
				var param = {};
				param.tableId = moudle.exports.tableId;
				param.serverSide = false;
				var url = basePath + "rfid/archivetype/getTableData";
				param.columns = [
							{"data":"name"},
							{"data":"remark"},
							{"data":"createTime"},
			...
			addRecordShow:function(){
				var url = basePath + "rfid/archivetype/addRecordShow";
				$("#main-content").load(url);
			},
			editRecordShow:function(id){
				var url = basePath + "rfid/archivetype/" + id +"/editRecordShow";
				$("#main-content").load(url);
			},
			deleteRecord:function(id){
				winUtil.confirm("操作确认","您确定删除该条记录吗?",function(){
					$.ajax({
						url:basePath + "rfid/archivetype/deleteArchiveType",
						data:{"id":id},
						type:"POST",
						dataType:"JSON",
						success:function(msg){
							if(msg.ok){
								util.showMsg("success","操作成功","您已经成功删除类型信息");
