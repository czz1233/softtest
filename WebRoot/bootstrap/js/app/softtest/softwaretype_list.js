...
define(function(require, exports, moudle) {
	...
	moudle.exports = {
			tableId:"softwaretype_table",
			tableInit:function(){
				var param = {};
				param.tableId = moudle.exports.tableId;
				param.serverSide = false;
				var url = basePath + "softtest/softwaretype/getTableData";
				param.columns = [
							{"data":"name"},
							{"data":"remark"},
							{"data":"createTime"},
			...
			addRecordShow:function(){
				var url = basePath + "softtest/softwaretype/addRecordShow";
				$("#main-content").load(url);
			},
			editRecordShow:function(id){
				var url = basePath + "softtest/softwaretype/" + id +"/editRecordShow";
				$("#main-content").load(url);
			},
			deleteRecord:function(id){
				winUtil.confirm("操作确认","您确定删除该条记录吗?",function(){
					$.ajax({
						url:basePath + "softtest/softwaretype/deleteSoftWareType",
						data:{"id":id},
						type:"POST",
						dataType:"JSON",
						success:function(msg){
							if(msg.ok){
								util.showMsg("success","操作成功","您已经成功删除软件类型信息");
