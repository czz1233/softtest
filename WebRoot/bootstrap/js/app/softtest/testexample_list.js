...
define(function(require, exports, moudle) {
	...
	moudle.exports = {
			tableId:"testexample_table",
			tableInit:function(){
				var param = {};
				param.tableId = moudle.exports.tableId;
				param.serverSide = false;
				var url = basePath + "softtest/testexample/getTableData";
				param.columns = [
							{"data":"title"},
							{"data":"testPlanStr"},
				...
				param.columnDefs = [{"render":function ( data, type, row) {
				                    	...
				                    	var searchButton = $("<label>&nbsp;&nbsp;<button type='button' class='but-row-search btn btn-minier btn-primary btn-round'><i class='fa fa-search bigger-110'></i>  查看</button><label>");
			...
			addRecordShow:function(){
				var url = basePath + "bootstrap/jsp/softtest/testexample_add_frame.jsp";
				$("#main-content").load(url);
			},
			editRecordShow:function(id){
				var url = basePath + "bootstrap/jsp/softtest/testexample_edit_frime.jsp?id="+id;
				$("#main-content").load(url);
			},
			searchRecord:function(id){
				var url = basePath + "softtest/testexample/search?id="+id;
				window.open(url,"_blank");
			},
			deleteRecord:function(id){
				winUtil.confirm("操作确认","您确定删除该条记录吗?",function(){
					$.ajax({
						url:basePath + "softtest/testexample/deleteTestExample",
						data:{"id":id},
						type:"POST",
						dataType:"JSON",
						success:function(msg){
							if(msg.ok){
								util.showMsg("success","操作成功","您已经成功删除软件信息");
