...
define(function(require, exports, moudle) {
	...
	moudle.exports = {
			tableId:"software_table",
			tableInit:function(){
				var param = {};
				param.tableId = moudle.exports.tableId;
				param.serverSide = false;
				var url = basePath + "softtest/software/getTableData";
				param.columns = [
							{"data":"name"},
							{"data":"type"},
							{"data":"vision"},
			...
			addRecordShow:function(){
				var url = basePath + "softtest/software/addRecordShow";
				$("#main-content").load(url);
			},
			editRecordShow:function(id){
				var url = basePath + "softtest/software/" + id +"/editRecordShow";
				$("#main-content").load(url);
			},
			deleteRecord:function(id){
				winUtil.confirm("操作确认","您确定删除该条记录吗?",function(){
					$.ajax({
						url:basePath + "softtest/software/deleteSoftWare",
						data:{"id":id},
						type:"POST",
						dataType:"JSON",
