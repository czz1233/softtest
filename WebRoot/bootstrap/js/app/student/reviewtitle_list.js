...
define(function(require, exports, moudle) {
	...
	moudle.exports = {
			tableId:"reviewtitle_table",
			tableInit:function(){
				var param = {};
				param.tableId = moudle.exports.tableId;
				param.serverSide = false;
				var url = basePath + "student/reviewtitle/getTableData";
				param.columns = [
							{"data":"title"},
							{"data":"subTitle"},
							{"data":"describle"},
				...
				param.columnDefs = [{"render":function ( data, type, row) {
				                    	...
				                    	var reviewButton = $("<label>&nbsp;&nbsp;<button type='button' class='but-row-reback btn btn-minier btn-success btn-round'><i class='fa fa-commenting-o bigger-110'></i>  回复</button><label>");
				                    	reviewButton.children("button").attr({"vid":row.id});
				                    	var html = editButton.html() + reviewButton.html() + deleteButton.html();
				                    	return html;
				                    },
				                    "targets":5}
			...
			addRecordShow:function(){
				var url = basePath + "student/reviewtitle/addRecordShow";
				$("#main-content").load(url);
			},
			editRecordShow:function(id){
				var url = basePath + "student/reviewtitle/" + id +"/editRecordShow";
				$("#main-content").load(url);
			},
			rebackRecord:function(id){
				var url = basePath + "student/reviewtitle/" + id +"/rebackRecord";
				$("#main-content").load(url);
			},
			deleteRecord:function(id){
				winUtil.confirm("操作确认","您确定删除该条记录吗?",function(){
					$.ajax({
						url:basePath + "student/reviewtitle/deleteReviewTitle",
						data:{"id":id},
						type:"POST",
						dataType:"JSON",
						success:function(msg){
							if(msg.ok){
								util.showMsg("success","操作成功","您已经成功删除发起话题信息");
			...
			listener:function(){
				...
				$("#" + moudle.exports.tableId).on('click', '.but-row-reback', function (event) {
					moudle.exports.rebackRecord($(event.currentTarget).attr("vid"));
				});
			},
			init:function(){
