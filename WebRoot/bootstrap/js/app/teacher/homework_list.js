...
define(function(require, exports, moudle) {
	...
	moudle.exports = {
			tableId:"homework_table",
			tableInit:function(){
				var param = {};
				param.tableId = moudle.exports.tableId;
				param.serverSide = false;
				var url = basePath + "teacher/homework/getTableData";
				param.columns = [
							{"data":"title"},
							{"data":"endTime"},
							{"data":"content"},
			...
			addRecordShow:function(){
				var url = basePath + "bootstrap/jsp/teacher/homework_add_frime.jsp";
				$("#main-content").load(url);
			},
			editRecordShow:function(id){
				//var url = basePath + "teacher/homework/" + id +"/editRecordShow";
				var url = basePath + "bootstrap/jsp/teacher/homework_edit_frime.jsp?id=" + id;
				$("#main-content").load(url);
			},
			deleteRecord:function(id){
				winUtil.confirm("操作确认","您确定删除该条记录吗?",function(){
					$.ajax({
						url:basePath + "teacher/homework/deleteHomeWork",
						data:{"id":id},
						type:"POST",
						dataType:"JSON",
						success:function(msg){
							if(msg.ok){
								util.showMsg("success","操作成功","您已经成功删除作业信息");
