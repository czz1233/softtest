...
define(function(require, exports, moudle) {
	...
	moudle.exports = {
			tableId:"course_table",
			tableInit:function(){
				var param = {};
				param.tableId = moudle.exports.tableId;
				param.serverSide = false;
				var url = basePath + "teacher/course/getTableData";
				param.columns = [
							{"data":"name"},
							{"data":"beginTime"},
							{"data":"endTime"},
			...
			addRecordShow:function(){
				var url = basePath + "teacher/course/addRecordShow";
				$("#main-content").load(url);
			},
			editRecordShow:function(id){
				var url = basePath + "teacher/course/" + id +"/editRecordShow";
				$("#main-content").load(url);
			},
			deleteRecord:function(id){
				winUtil.confirm("操作确认","您确定删除该条记录吗?",function(){
					$.ajax({
						url:basePath + "teacher/course/deleteCourse",
						data:{"id":id},
						type:"POST",
						dataType:"JSON",
						success:function(msg){
							if(msg.ok){
								util.showMsg("success","操作成功","您已经成功删除课程信息");
