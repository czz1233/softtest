...
define(function(require, exports, moudle) {
	...
	moudle.exports = {
			tableId:"bookrack_table",
			tableInit:function(){
				var param = {};
				param.tableId = moudle.exports.tableId;
				param.serverSide = false;
				var url = basePath + "story/bookrack/getTableData";
				param.columns = [
							{"data":"name"},
							{"data":"imageUrl"},
							{"data":"storyTypeStr"},
			...
			addRecordShow:function(){
				var url = basePath + "story/bookrack/addRecordShow";
				$("#main-content").load(url);
			},
			editRecordShow:function(id){
				var url = basePath + "story/bookrack/" + id +"/editRecordShow";
				$("#main-content").load(url);
			},
			deleteRecord:function(id){
				winUtil.confirm("操作确认","您确定删除该条记录吗?",function(){
					$.ajax({
						url:basePath + "story/bookrack/deleteBookRack",
						data:{"id":id},
						type:"POST",
						dataType:"JSON",
